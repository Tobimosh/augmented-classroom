...
class APIClient<T> {
  ...
  login = async (data: T) => {
    try {
      ...
      // Store tokens and expiration timestamp in local storage
      localStorage.setItem("access_token", access_token);
      localStorage.setItem("refresh_token", refresh_token);
  ...
  setupTokenRefresh = () => {
    setInterval(() => {
      this.checkTokenAndRefresh();
    }, 1000); // Check more frequently for better accuracy
  };

  checkTokenAndRefresh = async () => {
    const expirationTimestamp = parseInt(localStorage.getItem("token_expiration") || "0", 10);
    const currentTime = Date.now();

    if (expirationTimestamp - currentTime < 5 * 60 * 1000) {
      // Token is about to expire, initiate token refresh
      this.isRefreshing = true;
      try {
        const newAccessToken = await this.refreshToken();
